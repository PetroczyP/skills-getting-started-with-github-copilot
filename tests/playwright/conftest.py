    ...
    try:
        ...
        for activity_name in activities.keys():
            current_participants = set(
                activities[activity_name].get("participants", [])
            )
...
def api_helper():
    ...
    class APIHelper:
        ...
        @staticmethod
        def unregister(
            activity: str, email: str, lang: str = "en"
        ) -> requests.Response:
...
def pytest_configure(config):
    ...
    """
    config.addinivalue_line("markers", "e2e: End-to-end UI tests with Playwright")
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if "playwright" in str(item.fspath):
            ...
            has_test_id = any(
                marker.name == "test_id" for marker in item.iter_markers()
            )

            # Warn if missing test ID (for non-BDD tests)
            if (
                not has_test_id
                and "test_" in item.name
                and "bdd" not in str(item.fspath)
            ):
